...
def is_element_exist(driver, path):
    flag = True
    try:
        DW(driver, 20, 0.5).until(
            EC.visibility_of_element_located((By.XPATH, path)))
        return flag
    except (TimeoutError, TimeoutException):
        print('{} -- {} -- 等待20s未找到该元素，path：{}'.format(now(),
              threading.currentThread().name, path))
        flag = False
        return flag
    except Exception as e:
        print('{} -- {} -- 定位元素时出现未知异常，path：{}; Err:{}'.format(now(),
              threading.currentThread().name, path, e))
        flag = False
        return flag
...
def send_email(subject=r'Instagram脚本中止通知', content=r'Instagram 脚本已停止运行，详情请查看控制台运行日志。'):
    ...
    try:
        ...
        print('{} -- {} -- 邮箱登录成功，用户名：{}'.format(now(),
              threading.currentThread().name, sender))
    except Exception as err:
        print('{} -- {} -- 邮箱登录失败。ERR：{}'.format(now(),
              threading.currentThread().name, err))
        return
    try:
        server.sendmail(sender, recipient, msg.as_string())
        print('{} -- {} -- 邮件发送成功'.format(now(), threading.currentThread().name))
    except Exception as err:
        print('{} -- {} -- 邮件发送失败。ERR：{}'.format(now(),
              threading.currentThread().name, err))
        return
...
def open_browser(url):
    ...
    try:
        ...
        print('{} -- {} -- 启动浏览器失败,ERR: {}'.format(now(),
              threading.currentThread().name, e))
        raise
    try:
        driver.get(instagram_url)
        print("{} -- {} -- 访问Instagram".format(now(),
        ...
        print('{} -- {} -- 访问Instagram登录页面失败，ERR：{}'.format(now(),
              threading.currentThread().name, e))
...
def close_browser(driver):
    try:
        ...
        print('{} -- {} -- 没有实例化的浏览器，无需关闭'.format(now(),
              threading.currentThread().name))
        raise
    except Exception as e:
        print('{} -- {} -- 未知异常，关闭浏览器实例失败 {}'.format(now(),
              threading.currentThread().name, e))
        raise
...
def end_script(driver):
    ...
    try:
        ...
        print('{} -- {} -- 没有运行的浏览器实例，无需关闭'.format(now(),
              threading.currentThread().name))
    except Exception as e:
        print('{} -- {} -- 未知异常，退出浏览器实例失败  {}'.format(now(),
              threading.currentThread().name, e))
    # 退出脚本执行
...
def login_insta(driver, username, password):
    print('{} -- {} -- 准备登录的用户：{}'.format(now(),
          threading.currentThread().name, username))
    try:
        driver.title
    except RecursionError:
    ...
    if is_element_exist(driver, path_save_login):
        DW(driver, 20, 0.5).until(
            EC.element_to_be_clickable((By.XPATH, path_save_login)))
        ele_save_login = driver.find_element_by_xpath(path_save_login)
        # 保存登录信息
        ele_save_login.click()
        print('{} -- {} -- 是否保存登录信息：是'.format(now(),
              threading.currentThread().name))

    path_personal = '//*[@id="react-root"]/div/div/section/nav/div[2]/div/div/div[3]/div/div[5]/span/img'

    if is_element_exist(driver, path_personal):
        print('{} -- {} -- {} 已经登录成功'.format(now(),
              threading.currentThread().name, username))
    else:
        print('{} -- {} -- 未进入首页!未找到主页头像元素：{}'.format(now(),
              threading.currentThread().name, path_personal))
        return

    # 登录之后询问是否开启消息通知，此处选择“下次再说”
    path_handle_notice = '/html/body/div[5]/div/div/div/div[3]/button[2]'
    ...
    if is_element_exist(driver, path_notice_window):
        # print('11111')
        DW(driver, 20, 0.5).until(
            EC.element_to_be_clickable((By.XPATH, path_handle_notice)))
...
def logout_insta(driver):
    ...
    try:
        DW(driver, 20, 0.5).until(
            EC.visibility_of_element_located((By.XPATH, path_avatar)))
    except TimeoutError:
        print('{} -- {} -- 未找到个人中心头像元素，不需要执行退出登录'.format(now(),
              threading.currentThread().name))
        return
    except TimeoutException:
        print('{} -- {} -- 未找到个人中心头像元素，不需要执行退出登录'.format(now(),
        ...
        print('{} -- {} -- 未知错误，定位个人中心的头像元素时出错 {}'.format(now(),
              threading.currentThread().name, e))
        return
    # 点击头像
    ...
    DW(driver, 20, 0.5).until(
        EC.visibility_of_any_elements_located((By.XPATH, path_logout)))
...
def subscribe_user(url, user, pwd, target_users, frequency, target_num):
    try:
        ...
        print('{} -- {} -- 启动浏览器失败，退出脚本.ERR: {}'.format(now(),
              threading.currentThread().name, e))
        content = '{} -- {} -- 启动浏览器失败，退出脚本.ERR：{}'.format(
    ...
    for target_user in target_users:
        print('{} -- {} -- 目标用户的用户名{}'.format(now(),
              threading.currentThread().name, target_user))
        try:
            driver
        except RecursionError:
            print("{} -- {} -- 没有实例化的浏览器".format(now(),
                  threading.currentThread().name))
            login_insta(driver, user, pwd)
        ...
        print('{} -- {} -- 跳转到目标用户的主页{}'.format(now(),
              threading.currentThread().name, target_url))

        # 粉丝列表按钮
        path_fans_list = '//*[@id="react-root"]/div/div/section/main/div/header/section/ul/li[2]/a'

        for i in range(3):
            try:
                print('{} -- {} -- 等待粉丝列表按钮出现'.format(now(),
                      threading.currentThread().name))
                DW(driver, 20, 0.5).until(
                    EC.presence_of_element_located((By.XPATH, path_fans_list)))
                print('{} -- {} -- 粉丝列表按钮出现'.format(now(),
                      threading.currentThread().name))
                break
            except (TimeoutError, TimeoutException):
                print('{} -- {} -- Timeout! 未找到粉丝列表'.format(now(),
                      threading.currentThread().name))
                print('{} -- {} -- 判断当前页面是否为目标用户主页'.format(now(),
                      threading.currentThread().name))
                if driver.current_url != target_url:
                    print('{} -- {} -- 当前页面的url: {}'.format(now(),
                          threading.currentThread().name, driver.current_url))
                    print('{} -- {} -- 当前页面不是目标用户的主页'.format(now(),
                          threading.currentThread().name))
                    print('{} -- {} -- 重新跳转到目标用户主页'.format(now(),
                          threading.currentThread().name))
                    driver.get(target_url)
            if i == 2:
                print('{} -- {} -- 第3次加载目标用户主页，没有找到粉丝列表按钮'.format(now(),
                      threading.currentThread().name))
                print('{} -- {} -- 结束当前账号 {} 执行'.format(now(),
                      threading.currentThread().name, user))
                # 邮件通知
                content = '{} -- {} -- 结束当前账号 {} 执行'.format(
                    now(), threading.currentThread().name, user)
                send_email(content=content)
                close_browser(driver)
                return
        ...
        try:
            print('{} -- {} -- 点击粉丝列表按钮'.format(now(),
                  threading.currentThread().name))
            ele_fans_list = driver.find_element_by_xpath(path_fans_list)
            ele_fans_list.click()
        except ElementClickInterceptedException:
            if is_element_exist(driver, path_fans_list_title):
                print('{} -- {} -- 粉丝列表已打开'.format(now(),
                      threading.currentThread().name))
            else:
                print('{} -- {} -- 当前页面未找到粉丝列表按钮，结束当前账号 {} 执行'.format(now(),
                      threading.currentThread().name, user))
                # 邮件通知
                content = '{} -- {} -- 当前页面未找到粉丝列表按钮，结束当前账号 {} 执行'.format(
                    now(), threading.currentThread().name, user)
                send_email(content=content)
                close_browser(driver)
                return
        except NoSuchElementException:
            print('{} -- {} -- 未知异常导致粉丝列表按钮丢失，结束当前账号 {} 执行'.format(now(),
                  threading.currentThread().name, user))
            # 邮件通知
            content = '{} -- {} -- 未知异常导致粉丝列表按钮丢失，结束当前账号 {} 执行'.format(
                now(), threading.currentThread().name, user)
            send_email(content=content)
            close_browser(driver)
            return
        ...
        try:
            DW(driver, 5, 0.5).until(EC.presence_of_element_located(
                (By.XPATH, path_but_subscribe)))
        except (TimeoutError, TimeoutException):
            print('{} -- {} -- 粉丝列表中没有【关注】按钮'.format(now(),
                  threading.currentThread().name))
            print('{} -- {} -- 切换到另一个目标用户查找粉丝'.format(now(),
                  threading.currentThread().name))
            continue
        ...
        while True:
            ...
            for i in range(3):
                try:
                    # div的滚动条操作
                    # js = r"document.getElementsByClassName('isgrP')[0].scrollTop=10000000"
...
def div_scroll_top(driver, height):
    ...
    try:
        driver.execute_script(js)
        print('{} -- {} -- 滚动条向下滚动 {} 高度'.format(now(),
              threading.currentThread().name, height))
    except Exception as e:
        print('{} -- {} -- 执行js脚本出错。 \n js:【{}】 \n ERR:{}'.format(now(),
              threading.currentThread().name, js, e))
        raise
...
def unsubscribe(uname, pwd):
    ...
    if ele_following_number.text == 'NaN':
        print('{} -- {} -- 当前账号 {} 的following列表没有正在关注的人，退出脚本'.format(now(),
              threading.currentThread().name, uname))
        content = '{} -- {} -- 当前账号 {} 的following列表没有正在关注的人，退出脚本'.format(
            now(), threading.currentThread().name, uname)
        send_email(content=content)
        close_browser(driver)
        return
    ...
    print('{} -- {} -- 已经打开following列表'.format(now(),
          threading.currentThread().name))
    while True:
        try:
            # div的滚动条操作
            div_scroll_top(driver, 100)
            print('{} -- {} -- 加载列表下一页数据。。。'.format(now(),
                  threading.currentThread().name))
        except Exception as e:
            print(
            ...
            return
        try:
            DW(driver, 20).until_not(
                EC.presence_of_element_located((By.CLASS_NAME, '  By4nA')))
        except (TimeoutException, TimeoutError) as e:
            print('{} -- {} -- 加载列表下一页超时！等待20s，加载的loading还未消失。退出脚本'.format(now(),
                  threading.currentThread().name))
            # 邮件通知
            content = '{} -- {} -- 操作列表滚动条异常，退出当前账号 {} 的执行！{}'.format(now(), threading.currentThread().name,
                                                                      user, e)
        ...
        path_but_following = '//button[text()="已关注" or text()="Following"]'
        DW(driver, 20).until(EC.presence_of_element_located(
            (By.XPATH, path_but_following)))
        ...
        if number_but_following == 0:
            print('{} -- {} -- 当前账号 {} following列表中没有正在关注的人，退出脚本'.format(now(),
                  threading.currentThread().name, uname))
        ...
        for i in range(number_but_following):
            ...
            DW(driver, 20).until(EC.element_to_be_clickable(
                (By.XPATH, path_confirm_unsubscribe)))
            ele_confirm_unsubscribe = driver.find_element_by_xpath(
                path_confirm_unsubscribe)
            ele_confirm_unsubscribe.click()
            print('{} -- {} -- 点击【停止关注】按钮'.format(now(),
                  threading.currentThread().name))
            sleep_time = random.randint(3, 10)
            sleep(sleep_time)
            print('{} -- {} -- 随机休息 {} s'.format(now(),
                  threading.currentThread().name, sleep_time))
    ...
    while True:
        for comment in comments:
            ...
            for ele_post in ele_posts:
                ...
                print('{} -- {} -- 打开当前页第 {} 个帖子'.format(now(),
                      threading.currentThread().name, n))
                ele_post.click()
                # 点赞按钮
                ...
                if is_liked in ['赞', 'Like']:
                    ele_but_like.click()
                    sleep(1)
                    print('{} -- {} -- 给新帖子点赞'.format(now(),
                          threading.currentThread().name))
                    try:
                        ele_comment_box = driver.find_element_by_xpath(
                            path_comment_box)
                        ...
                        ele_new_comment_box = driver.find_element_by_xpath(
                            path_comment_box)
                        ele_new_comment_box.clear()
                        comment = 'It is great! There are also something amazing here: @{}'.format(
                            comment_link)
                        ele_new_comment_box.send_keys(comment)
                        print('{} -- {} -- 给新帖子评论 {}'.format(now(),
                              threading.currentThread().name, comment))
                        ele_submit_comment = driver.find_element_by_xpath(
                            path_submit_comment)
                        ele_submit_comment.click()
                    except NoSuchElementException:
                        print('{} -- {} -- 该帖子未开放评论'.format(now(),
                              threading.currentThread().name))
                # 关闭帖子
                path_but_close_post = r'//*[name()="svg"][contains(@aria-label,"关闭") or contains(@aria-label,"Close")]'
                ele_but_close_post = driver.find_element_by_xpath(
                    path_but_close_post)
                ele_but_close_post.click()
                print('{} -- {} -- 关闭帖子'.format(now(),
                      threading.currentThread().name))
                n += 1
                sleep(2)
                # 刷新当前页面以获取新帖子
...
if __name__ == '__main__':
    ...
    for user in users:
        ...
        print('*' * 6)
        print('当前时间为：{} ，开始操作账号： {}'.format(
            time.strftime("%H:%M:%S", time.localtime()), uname))
